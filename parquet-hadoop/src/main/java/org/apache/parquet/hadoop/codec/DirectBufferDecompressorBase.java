...
public abstract class DirectBufferDecompressorBase implements Decompressor {
  ...
  // If decompresses can get uncompressed length without decompressing. Otherwise returns -1;
  protected abstract int getUncompressedLength(ByteBuffer src) throws IOException;
  // decompresses using Direct byte buffers
  protected abstract int decompressDirect(ByteBuffer src, ByteBuffer dest) throws IOException;
  ...
  public synchronized int decompress(byte[] buffer, int off, int len) throws IOException {
    ...
    if (!outputBuffer.hasRemaining()) {
      ...
      if(decompressedSize < 0){
        // if we don't have decompressed size we use the output buffer length
      ...
      outputBuffer.clear();
      int size = decompressDirect(inputBuffer, outputBuffer);
